...
FeedController extends Controller{
  ...
  public function index(){
    if(count($this->user->empresas)>0){
      ...
      return view('feeds.index', ['companyStatuses' => EstadoEmpresa::orderBy('created_at', 'desc')->paginate(10), 'myCompanies' => $this->user->empresas, 'bannersRandom' => BannerData::orderByRaw('RAND()')->take(3)->get(), 'userSession' => $this->user, 'companies' => Empresa::select('id','nombre','imagen_perfil')->orderByRaw('RAND()')->take(8)->get()] ); //cambiar EstadoEmpresa por CompanyStatus
    }else{
      return view('feeds.index', ['companyStatuses' => EstadoEmpresa::orderBy('created_at', 'desc')->paginate(10), 'bannersRandom' => BannerData::orderByRaw('RAND()')->take(3)->get(), 'userSession' => $this->user, 'companies' => Empresa::select('id','nombre','imagen_perfil')->orderByRaw('RAND()')->take(8)->get()]);
    }
  }
