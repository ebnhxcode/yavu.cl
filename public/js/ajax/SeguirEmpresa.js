...
/*FUNCIONES Y PROCEDIMIENTOS*/
	function Seguir(company_id, btn){
		...
		var route = "http://192.168.1.42/seguirempresa";
		$.ajax({
			url: route,
	...
	function NoSeguir(company_id){
		...
		var token = $("#token").val();
		var route = "http://192.168.1.42/noseguirempresa/";
		$.ajax({
			url: route,
			headers: {'X-CSRF-TOKEN': token},
			type: 'POST',
	...
	function ContarSeguidores(company_id){
		...
		var company_id = $("#company_id").val();
		$.ajax({
			url: "http://192.168.1.42/contarseguidores/"+company_id+"/"+user_id,
			type: 'GET',
			dataType: 'json',
			cache: false,
			async: true,
	...
	function VerificarSeguidores(company_id){
		...
		var company_id = $("#company_id").val();
		$.ajax({
			url: "http://192.168.1.42/verificarseguidores/"+company_id+"/"+user_id,
			type: 'GET',
			dataType: 'json',
			cache: false,
			async: true,
