...
/*FUNCIONES Y PROCEDIMIENTOS*/
	function ListarEmpresasUsuarios(){
		...
		var route = "http_//yavu.cl/infoempresas/"+user_id;
		var Pendiente = false;
		var Contador = 0;
			...
			$(res).each(function(key,value){
				...
				if(value.estado === "Pendiente"){
					...
					if (value.imagen_portada === ""){
						ImagenPortada = "<div class='well well-sm'>No tienes un banner personal, puedes subir una editando la información de tu empresa <a class='btn-link' href='http_//yavu.cl/empresas/"+value.id+"/edit'>aquí</a></div>";
					}else{
						...
						+'<div id="collapse'+Contador+'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading'+Contador+'">'
							+'<div class="panel-body">'
								+"<div class='thumbnail card' style='border: 0px;'>"
									+ImagenPortada+"<br>"
									+"<span><a class='btn btn-primary btn-sm' href='http_//yavu.cl/empresa/"+value.nombre+"/'>Perfil</a></span>&nbsp;"
									+"<span><a class='btn btn-primary btn-sm' href='http_//yavu.cl/empresas/"+value.id+"/edit'>Editar información</a></span>&nbsp;"
									+"<p>...</p><br>"
								+"</div>"
							+'</div>'
						+'</div>'
